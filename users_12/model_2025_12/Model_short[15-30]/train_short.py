...
# ========== 1. 路径 ==========
# [修改] 使用绝对路径，避免相对路径带来的混淆
data_pkl = "/home/yanglanqi/trajectory/zx_users_12/model_2025_12/preprocessed_data/zx_station_dataset.pkl"

output_dir = "/home/yanglanqi/trajectory/zx_users_12/model_2025_12/Model_short"
os.makedirs(output_dir, exist_ok=True)
save_ckpt = os.path.join(output_dir, "model_51users.pt")
...
split_dir = "/home/yanglanqi/trajectory/zx_users_12/model_2025_12/Model_short/SPLIT_DATASET"
...
# [修复] 参数去重逻辑，解决 "optimizer contains a parameter group with duplicate parameters" 警告
...
for epoch in range(epochs):
    ...
    for step, batch in enumerate(loader):
        ...
        if valid_pos.any():
            ...
            acc50 = torch.tensor(0.0, device=device)
            acc100 = torch.tensor(0.0, device=device)
        ...
        if step % 100 == 0:
            print(f"Epoch {epoch:02d} | Step {step:04d} | "
                  ...
                  f"Acc={accuracy.item():.2%} | A@20={acc20.item():.2%} | A@50={acc50.item():.2%} | A@100={acc100.item():.2%} | "
                  f"MAE(m)={mae.item():.3f} | MSE(m^2)={mse.item():.3f}")

    # ========== 6. 验证评估 ==========
    transformer.eval(); diffusion.eval()
    val_loss_sum = 0.0; val_loss_cls_sum = 0.0
    val_acc_sum = 0.0; val_mae_sum = 0.0; val_mse_sum = 0.0
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            if valid_pos.any():
                ...
                loss_cls = torch.tensor(0.0, device=device)
                acc = 0.0; mae = 0.0; mse = 0.0
                acc20 = 0.0; acc50 = 0.0; acc100 = 0.0
    ...
    print(f"[VAL] Epoch {epoch:02d} | val_loss={avg_val_loss:.4f} | "
          f"val_loss_cls={avg_val_loss_cls:.4f} | val_acc={avg_val_acc:.2%} | A@20={avg_val_acc20:.2%} | A@50={avg_val_acc50:.2%} | A@100={avg_val_acc100:.2%} | "
          f"val_MAE(m)={avg_val_mae:.3f} | val_MSE(m^2)={avg_val_mse:.3f}")

# ========== 7. 测试评估 ==========
transformer.eval(); diffusion.eval()
test_loss_sum = 0.0; test_loss_cls_sum = 0.0
test_acc_sum = 0.0; test_mae_sum = 0.0; test_mse_sum = 0.0
test_acc20_sum = 0.0; test_acc50_sum = 0.0; test_acc100_sum = 0.0
test_count = 0
with torch.no_grad():
    for batch in test_loader:
        ...
        test_mae_sum += mae
        test_mse_sum += mse
        test_acc20_sum += acc20
        test_acc50_sum += acc50
        test_acc100_sum += acc100
        test_count += 1
...
avg_test_acc20 = test_acc20_sum / max(test_count, 1)
avg_test_acc50 = test_acc50_sum / max(test_count, 1)
avg_test_acc100 = test_acc100_sum / max(test_count, 1)

print(f"[TEST] avg_loss={avg_test_loss:.4f} | avg_loss_cls={avg_test_loss_cls:.4f} | "
      f"avg_acc={avg_test_acc:.2%} | A@20={avg_test_acc20:.2%} | A@50={avg_test_acc50:.2%} | A@100={avg_test_acc100:.2%} | "
      f"avg_MAE(m)={avg_test_mae:.3f} | avg_MSE(m^2)={avg_test_mse:.3f}")
