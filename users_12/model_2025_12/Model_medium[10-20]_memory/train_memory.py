...
output_dir = "/home/yanglanqi/trajectory/zx_users_12/model_2025_12/Model_medium[10-20]_memory"
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            if valid_pos.any():
                ...
                acc20 = (dists <= 20.0).float().mean().item()
                acc50 = (dists <= 50.0).float().mean().item()
                acc100 = (dists <= 100.0).float().mean().item()
                acc200 = (dists <= 200.0).float().mean().item()
                acc500 = (dists <= 500.0).float().mean().item()
            else:
                 loss_cls = torch.tensor(0.0, device=device)
