...
# ========== 配置 ==========
MODEL_PATH = "trajectory/geolife_clustering-master/geolife_clustering-master/Model_construction_mins/model_minutes.pt"
DATA_PKL = "trajectory/geolife_clustering-master/geolife_clustering-master/Model_pre/cluster_token_dataset.pkl"
OUTPUT_DIR = "trajectory/geolife_clustering-master/geolife_clustering-master/Model_construction_mins/Evalution"  # 结果输出目录
...
def evaluate_and_visualize():
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            # 2. 预测
            # 对于连续缺失片段，单步预测效果较差，改用迭代采样 (Iterative Sampling)
            # steps=50 在速度和质量间取得平衡，token_obs 用于保持非缺失部分的观测值
            pred_tokens = diffusion.sample(z_t, mask, token_obs=token_masked_input, steps=50)
    ...
    # --- 汇总报告 ---
    print("\n" + "=" * 60)
    print(f"最终评估报告 (Test Set - Iterative Sampling)")
    print("=" * 60)
...
def restore_full_dataset():
    ...
    with torch.no_grad():
        for batch in dl:
            ...
            # 使用迭代采样进行修复，确保连续缺失段的连贯性
            pred_ids = diffusion.sample(z_t, mask_vec, token_obs=token_id, steps=50)
